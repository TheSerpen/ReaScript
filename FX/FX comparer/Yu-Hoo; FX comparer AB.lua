...
if Ret>0 then
  ...
  if Item==-1 then
    if reaper.TrackFX_GetOpen(Track,FX) or IgnoreVisible then
      ...
      Ret, FXData=reaper.GetProjExtState(0,"YUHOO_FX_A/B_COMPARER",GUID)
      if FXData~="" then
        YH.SetTrackFXChunk(Track,FX,FXData)
      ...
      reaper.Undo_EndBlock2(0,"FX comparer A/B: "..Name..": "..FX,-1)
    end
  else
    ...
    if reaper.TakeFX_GetOpen(Take,FX) or IgnoreVisible then
      ...
      Ret, FXData=reaper.GetProjExtState(0,"YUHOO_FX_A/B_COMPARER",GUID)
      if FXData~="" then
        YH.SetTakeFXChunk(Take,FX,FXData)
        reaper.SetProjExtState(0,"YUHOO_FX_A/B_COMPARER",GUID,FXChunk)
      else
        reaper.SetProjExtState(0,"YUHOO_FX_A/B_COMPARER",GUID,FXChunk)
        if SetToDefault then
      ...
      FX=YH.GetOnlyFXName(FX)
      reaper.Undo_EndBlock2(0,"FX comparer A/B: Item on track "..Name..": "..FX,-1)
    end
  end
end
